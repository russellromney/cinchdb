...
def feature_add_reviews(db):
    ...
    feature_db.create_table("reviews", [
    ...
    if feature_db.is_local:
        feature_db.columns.add_column("products", 
            Column(name="review_count", type="INTEGER", nullable=True)
        )
        feature_db.columns.add_column("products",
            Column(name="average_rating", type="REAL", nullable=True)
        )
    ...
    # Insert test data
    user = feature_db.insert("users", {
        "email": "reviewer@example.com",
        "name": "Test Reviewer",
        "role": "customer"
    })
    
    product = feature_db.insert("products", {
        "name": "Test Product",
        "price": 29.99,
        "stock": 100,
        "review_count": 0,
    ...
    feature_db.update("products", product["id"], {
...
def feature_add_inventory(db):
    ...
    feature_db.create_table("inventory_movements", [
        Column(name="product_id", type="TEXT"),
    ...
    for m in movements:
        feature_db.insert("inventory_movements", {
            "product_id": product["id"],
            "quantity": abs(m["qty"]),
    ...
    if changes_path.exists():
        ...
        for i, change in enumerate(changes, 1):
            ...
            if change_type == "CREATE_TABLE":
                ...
                print(f"  {i}. Added column '{change['details']['column']['name']}' to '{change['details']['table']}'")
