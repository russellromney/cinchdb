...
def create_source_distribution():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        src_dir = Path("src")
        if src_dir.exists():
            shutil.copytree(src_dir, temp_project / "src", 
...
def main():
    ...
    parser.add_argument("--skip-existing", action="store_true", help="Skip if version already exists")
    parser.add_argument("--no-clean", action="store_true", help="Don't clean build artifacts first")
    parser.add_argument("--check-only", action="store_true", help="Only check package contents, don't build")
    ...
    if not (args.publish or args.testpypi):
        ...
        print("  - To publish to TestPyPI: python scripts/build_and_publish.py --testpypi")
        print("  - To publish to PyPI: python scripts/build_and_publish.py --publish")
    else:
        print(f"  - Package published to {repository}")
        print("  - Install with: pip install cinchdb" + (" -i https://test.pypi.org/simple/" if args.testpypi else ""))
