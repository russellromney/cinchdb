...
def serve(
    ...
    create_key: bool = typer.Option(False, "--create-key", help="Create an API key on startup")
):
...
def create_key(
    ...
    permissions: str = typer.Option("read", "--permissions", "-p", help="Permissions (read/write)"),
    project_dir: Optional[Path] = typer.Option(None, "--project-dir", "-d", help="Project directory")
):
...
def list_keys(
    ...
    for key in keys:
        ...
        branches = f"Branches: {', '.join(key.branches)}" if key.branches else "All branches"
        console.print(f"\n{key.name} - {status}")
