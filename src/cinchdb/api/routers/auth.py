...
async def create_api_key(
    ...
    if auth.api_key.permissions != "write":
        raise HTTPException(status_code=403, detail="Write permission required to create API keys")
    
    manager = APIKeyManager(project_dir)
    
    # Validate permissions
    if request.permissions not in ["read", "write"]:
        raise HTTPException(status_code=400, detail="Invalid permissions. Must be 'read' or 'write'")
...
async def list_api_keys(
    ...
    if auth.api_key.permissions != "write":
        raise HTTPException(status_code=403, detail="Write permission required to list API keys")
...
async def revoke_api_key(
    ...
    if auth.api_key.permissions != "write":
        raise HTTPException(status_code=403, detail="Write permission required to revoke API keys")
