...
class CinchDB:
    ...
    def tables(self) -> "TableManager":
        ...
        if self._table_manager is None:
            from cinchdb.managers.table import TableManager
            self._table_manager = TableManager(self.project_dir, self.database, self.branch, self.tenant)
    ...
    def columns(self) -> "ColumnManager":
        ...
        if self._column_manager is None:
            from cinchdb.managers.column import ColumnManager
            self._column_manager = ColumnManager(self.project_dir, self.database, self.branch, self.tenant)
    ...
    def views(self) -> "ViewModel":
        ...
        if self._view_manager is None:
            from cinchdb.managers.view import ViewModel
            self._view_manager = ViewModel(self.project_dir, self.database, self.branch, self.tenant)
    ...
    def tenants(self) -> "TenantManager":
        ...
        if self._tenant_manager is None:
            from cinchdb.managers.tenant import TenantManager
            self._tenant_manager = TenantManager(self.project_dir, self.database, self.branch)
    ...
    def query(self, sql: str, params: Optional[List[Any]] = None) -> List[Dict[str, Any]]:
        ...
        if self.is_local:
            if self._query_manager is None:
                from cinchdb.managers.query import QueryManager
                self._query_manager = QueryManager(self.project_dir, self.database, self.branch, self.tenant)
    ...
    def create_table(self, name: str, columns: List[Column]) -> None:
        ...
        if self.is_local:
            ...
            self._make_request(
                "POST", 
                "/tables", 
                json={"name": name, "columns": columns_data}
            )
