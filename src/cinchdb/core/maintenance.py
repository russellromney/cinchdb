...
def get_maintenance_info(project_root: Path, database: str, branch: str) -> Optional[Dict[str, Any]]:
...
def check_maintenance_mode(project_root: Path, database: str, branch: str) -> None:
    ...
    if is_branch_in_maintenance(project_root, database, branch):
        ...
        reason = info.get("reason", "Maintenance in progress") if info else "Maintenance in progress"
