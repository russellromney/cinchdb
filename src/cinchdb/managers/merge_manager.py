...
class MergeManager:
    ...
    def can_merge(self, source_branch: str, target_branch: str) -> Dict[str, Any]:
        ...
        source_only, target_only = self.comparator.get_divergent_changes(source_branch, target_branch)
        if not source_only:
            return {
                "can_merge": False,
        ...
        # Check merge type
        is_fast_forward = self.comparator.can_fast_forward_merge(source_branch, target_branch)
    ...
    def _merge_branches_internal(self, source_branch: str, target_branch: str, force: bool = False, dry_run: bool = False) -> Dict[str, Any]:
        ...
        if not merge_check["can_merge"]:
            if not force:
                ...
                raise MergeError(f"Cannot force merge due to conflicts: {', '.join(merge_check['conflicts'])}")
        
        # Get changes to merge
        source_only, _ = self.comparator.get_divergent_changes(source_branch, target_branch)
        if not source_only:
            return {
                "success": True,
        ...
        if dry_run:
            ...
            sql_statements = self._collect_sql_statements(ordered_changes, target_branch)
            return {
                "success": True,
                "dry_run": True,
    ...
    def merge_branches(self, source_branch: str, target_branch: str, force: bool = False, dry_run: bool = False) -> Dict[str, Any]:
        ...
        if target_branch == "main":
            raise MergeError("Cannot merge directly into main branch. Main branch is protected.")
        
        # Use internal method for actual merge
        return self._merge_branches_internal(source_branch, target_branch, force, dry_run)
    
    def _apply_changes_to_branch(self, changes: List[Change], target_branch: str) -> None:
        """Apply changes to target branch atomically.
        ...
        """
        target_tracker = ChangeTracker(self.project_root, self.database_name, target_branch)
        target_applier = ChangeApplier(self.project_root, self.database_name, target_branch)
        ...
        try:
            ...
            for change_id in applied_changes:
                try:
                    ...
                    target_tracker.remove_change(change_id) if hasattr(target_tracker, 'remove_change') else None
                except Exception:
    ...
    def merge_into_main(self, source_branch: str, dry_run: bool = False) -> Dict[str, Any]:
        ...
        main_only, source_only = self.comparator.get_divergent_changes("main", source_branch)
        if main_only:
            raise MergeError(
    ...
    def get_merge_preview(self, source_branch: str, target_branch: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "common_ancestor": self.comparator.find_common_ancestor(source_branch, target_branch)
        }