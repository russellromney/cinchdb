...
class TestCLIIntegration:
    ...
    def test_branch_operations(self, temp_project):
        ...
        result = self.run_in_project(["branch", "create", "feature", "--source", "main"], temp_project)
        assert result.exit_code == 0
    ...
    def test_table_operations(self, temp_project):
        ...
        self.run_in_project(["init"], temp_project)
        self.run_in_project(["branch", "create", "feature", "--source", "main"], temp_project)
    ...
    def test_column_operations(self, temp_project):
        ...
        # Add column
        result = self.run_in_project(["column", "add", "users", "age", "INTEGER"], temp_project)
        assert result.exit_code == 0
    ...
    def test_merge_workflow(self, temp_project):
        ...
        self.run_in_project(["column", "add", "products", "description", "TEXT"], temp_project)
        
        # Preview merge
        result = self.run_in_project(["branch", "merge", "feature", "--target", "main", "--preview"], temp_project)
        assert result.exit_code == 0
        assert "Merge Preview" in result.stdout
        assert "feature → main" in result.stdout
        
        # Merge into main
        result = self.run_in_project(["branch", "merge-into-main", "feature"], temp_project)
        assert result.exit_code == 0
    ...
    def test_error_handling(self, temp_project):
        ...
        output = result.stdout + result.stderr + str(result.exception) if result.exception else ""
        ...
        result = self.run_in_project(["column", "add", "nonexistent", "col", "TEXT"], temp_project)
        assert result.exit_code != 0
        output = result.stdout + result.stderr + str(result.exception) if result.exception else ""
