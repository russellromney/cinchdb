...
class TestEmptyTenantSchema:
    ...
    def test_empty_tenant_created_with_schema(self, test_project):
        ...
        # Create a table in main tenant using CinchDB
        db = CinchDB(database="testdb", branch="main", project_dir=test_project)
        ...
        db.create_tenant("tenant1", lazy=True)

        # Use TenantManager for internal checks
        tenant_manager = TenantManager(ConnectionContext(project_root=test_project, database="testdb", branch="main"))
    ...
    def test_empty_tenant_gets_schema_updates(self, test_project):
        ...
        # Use managers for internal schema update testing
        tenant_manager = TenantManager(ConnectionContext(project_root=test_project, database="testdb", branch="main"))
    ...
    def test_lazy_tenant_uses_updated_empty(self, test_project):
        ...
        # Use managers for internal schema testing
        tenant_manager = TenantManager(ConnectionContext(project_root=test_project, database="testdb", branch="main"))
        change_tracker = ChangeTracker(test_project, "testdb", "main")
        change_applier = ChangeApplier(test_project, "testdb", "main")
        ...
        db.create_tenant("tenant2", lazy=True)
    ...
    def test_empty_tenant_excluded_from_user_listings(self, test_project):
        ...
        # Create some tenants using CinchDB
        db = CinchDB(database="testdb", branch="main", project_dir=test_project)
        db.create_tenant("tenant1", lazy=True)
        db.create_tenant("tenant2", lazy=False)
