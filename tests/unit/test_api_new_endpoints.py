...
class TestNewAPIFunctionality:
    ...
    def test_branch_operations_imports(self):
        ...
        from cinchdb.api.routers.branches import BranchComparisonResult, MergeCheckResult
    ...
    def test_pydantic_models_structure(self):
        ...
        from cinchdb.api.routers.data import CreateDataRequest, UpdateDataRequest, BulkCreateRequest
        from cinchdb.api.routers.codegen import GenerateModelsRequest, CodegenLanguage
        from cinchdb.api.routers.branches import MergeBranchRequest, BranchComparisonResult
    ...
    def test_endpoint_count_validation(self):
        ...
        branch_endpoints = [r for r in all_routes if '/branches/' in r.path and any(op in r.path for op in ['compare', 'merge', 'can-merge'])]
        assert len(branch_endpoints) >= 3, f"Expected at least 3 new branch endpoints, got {len(branch_endpoints)}"
