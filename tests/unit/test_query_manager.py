...
class TestQueryManager:
    ...
    def query_manager(self, temp_project):
        ...
        # Insert test users
        users = [
            QueryTestUser(name="Alice Smith", email="alice@example.com", age=30, active=1),
            QueryTestUser(name="Bob Johnson", email="bob@example.com", age=25, active=1),
            QueryTestUser(name="Carol Davis", email="carol@example.com", age=35, active=0),
            QueryTestUser(name="David Wilson", email="david@example.com", age=28, active=1)
        ]
    ...
    def test_execute_one(self, query_manager):
        ...
        result = query_manager.execute_one("SELECT * FROM users WHERE email = 'alice@example.com'")
    ...
    def test_execute_non_query(self, query_manager):
        ...
        affected = query_manager.execute_non_query(
            ...
            ("test-id", "Eve Brown", "eve@example.com", 40, 1, datetime.now(), datetime.now())
        )
        assert affected == 1
    ...
    def test_execute_many(self, query_manager):
        ...
        params_list = [
            ("id1", "User1", "user1@example.com", 20, 1, datetime.now(), datetime.now()),
            ("id2", "User2", "user2@example.com", 21, 1, datetime.now(), datetime.now()),
            ("id3", "User3", "user3@example.com", 22, 1, datetime.now(), datetime.now())
        ]
        ...
        # Verify insertions
        results = query_manager.execute("SELECT * FROM users WHERE age BETWEEN 20 AND 22")
        assert len(results) == 3
    ...
    def test_transaction_rollback(self, query_manager):
        ...
        initial_count = query_manager.execute("SELECT COUNT(*) as count FROM users")[0]["count"]
        ...
        final_count = query_manager.execute("SELECT COUNT(*) as count FROM users")[0]["count"]
    ...
    def test_null_handling(self, query_manager):
        ...
        query_manager.execute_non_query(
            "INSERT INTO users (id, name, email, age, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
            ("null-test", "Null User", "null@example.com", 99, None, datetime.now(), datetime.now())
        )
