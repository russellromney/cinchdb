...
class TestSQLValidator:
    ...
    def test_allow_multiple_statements_flag(self):
        ...
        is_valid, error, operation = validate_sql_query(query, allow_multiple_statements=True)
    ...
    def test_blocked_cte_with_ddl(self):
        ...
        query = "WITH temp AS (CREATE TABLE test AS SELECT * FROM users) SELECT * FROM temp"
        is_valid, error, operation = validate_sql_query(query)
        assert is_valid is False
        assert "CTE" in error
