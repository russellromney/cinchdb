...
class TestViewModel:
    ...
    def test_create_view_complex(self, managers, temp_project):
        ...
        with DatabaseConnection(db_path) as conn:
            # Insert test data
            conn.execute("INSERT INTO users (id, name, email, created_at) VALUES (?, ?, ?, datetime('now'))", 
                        ("1", "John", "john@example.com"))
            conn.execute("INSERT INTO posts (id, title, user_id, created_at) VALUES (?, ?, ?, datetime('now'))", 
                        ("1", "Post 1", "1"))
            conn.execute("INSERT INTO posts (id, title, user_id, created_at) VALUES (?, ?, ?, datetime('now'))", 
                        ("2", "Post 2", "1"))
            conn.commit()
    ...
    def test_update_view(self, managers, temp_project):
        ...
        # Create initial view
        managers["view"].create_view("published_posts", "SELECT * FROM posts WHERE published = 1")
    ...
    def test_view_with_parameters(self, managers):
        ...
        managers["view"].create_view("recent_users", 
                                   "SELECT * FROM users WHERE created_at > date('now', '-30 days')")
        
        managers["view"].create_view("recent_active_users",
                                   "SELECT * FROM recent_users WHERE active = 1")
